...
class HFAutoModelForTextPrediction(nn.Module):
    ...
    def forward(
        ...
        if len(text_token_ids.shape) == 1:
            text_token_ids = text_token_ids.view(1, -1)
            text_valid_length = torch.tensor([text_valid_length])
        steps = torch.arange(0, text_token_ids.shape[1], device=text_valid_length.device).type_as(text_valid_length)
        # text_masks = (steps.reshape((1, -1)) < text_valid_length.reshape((-1, 1))).type_as(text_token_ids)
        text_masks = steps < text_valid_length.unsqueeze(1)
        text_masks = text_masks.to(text_token_ids.device)
        outputs = self.model(
